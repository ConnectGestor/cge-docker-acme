...
show_usage() {
    echo -e     "Uso: \n\n      curl -sSL https://raw.githubusercontent.com/ConnectGestor/cge-docker-acme/main/update.sh | sudo bash\n\n"
    echo -e "Exemplo: \n\n      curl -sSL https://raw.githubusercontent.com/ConnectGestor/cge-docker-acme/main/update.sh | sudo bash\n\n"
}
...
if [ -f docker-compose-acme.yaml ] && [ -f .env-backend-acme ] && [ -n "${BACKEND_PUBLIC_VOL}" ] && [ -n "${BACKEND_PRIVATE_VOL}" ] && [ -n "${POSTGRES_VOL}" ]; then
   ...
   docker volume create --name cge-docker-acme_backend_public || exit 1
   docker run --rm -v ${BACKEND_PUBLIC_VOL}:/from -v cge-docker-acme_backend_public:/to alpine ash -c "cd /from ; cp -a . /to"

   docker volume create --name cge-docker-acme_backend_private || exit 1
   docker run --rm -v ${BACKEND_PRIVATE_VOL}:/from -v cge-docker-acme_backend_private:/to alpine ash -c "cd /from ; cp -a . /to"

   docker volume create --name cge-docker-acme_postgres_data || exit 1
   docker run --rm -v ${POSTGRES_VOL}:/from -v cge-docker-acme_postgres_data:/to alpine ash -c "cd /from ; cp -a . /to"
...
if [ -d cge-docker-acme ] && [ -f cge-docker-acme/docker-compose.yaml ] ; then
  ...
  cd /root/cge-docker-acme || exit 1
else
  cd /home/${SUDO_USER}/cge-docker-acme || exit 1
fi

echo "Working on $PWD/cge-docker-acme folder"
